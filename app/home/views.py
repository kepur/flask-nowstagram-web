import hashlib
import random,json
from . import home
from flask import render_template,redirect,request,flash,get_flashed_messages
from app.admin.models import Image,User,db
from flask_login import login_user,logout_user,login_required,current_user



@home.route('/')
def index():
    images =Image.query.order_by('id').limit(10).all()
    return render_template('index.html',images=images)

@home.route('/image/<int:image_id>/')
def image(image_id):
    image=Image.query.get(image_id)
    if image==None:
        return redirect('/')
    return render_template('pageDetail.html',image=image)

# # 用户详情页
@home.route('/profile/<int:user_id>/')
@login_required
def profile(user_id):
    user = User.query.get(user_id)
    if user == None:
        return redirect('/')
    paginate = Image.query.filter_by(user_id=user_id).order_by(db.desc(Image.id)).paginate(page=1, per_page=3, error_out=False)
    return render_template('profile.html', user=user, images=paginate.items, has_next=paginate.has_next)

# # 处理异步交互
@home.route('/profile/images/<int:user_id>/<int:page>/<int:per_page>/')
def user_images(user_id, page, per_page):
    paginate = Image.query.filter_by(user_id=user_id).order_by(db.desc(Image.id)).paginate(page=page, per_page=per_page, error_out=False)
    map = {'has_next': paginate.has_next}
    images = []
    for image in paginate.items:
        imgvo = {'id': image.id, 'url': image.url, 'comment_count': len(image.comments)}
        images.append(imgvo)

    map['images'] = images
    return json.dumps(map)

#登陆注册页
@home.route('/regloginpage/')
def regloginpage():
    msg=''
    for m in get_flashed_messages(with_categories=False,category_filter=['reglogin']):
        msg=msg+m
    return render_template('login.html',msg=msg,next=request.values.get('next'))



###消息重定向
def redirect_wich_msg(taget,msg,category):
    if msg != None:
        flash(msg,category=category)
        return redirect(taget)

@home.route('/reg/',methods=['GET','POST'])
def reg():
    #request.args
    #request.form
    username=request.values.get('username').strip()
    password=request.values.get('password').strip()
    user=User.query.filter_by(username=username).first()
    if username == '' or password =='':
        return redirect_wich_msg('/regloginpage/',u'用户名或者密码不能为空','reglogin')
    if user != None:
        return redirect_wich_msg('/regloginpage/',u'用户名已经存在','reglogin')
    salt='.'.join(random.sample('0123456789abcdefghABCDEFGHIJKLMN',10))
    m=hashlib.md5()
    m.update((password+salt).encode("utf8"))
    password=m.hexdigest()
    user=User(username,password,salt)
    db.session.add(user)
    db.session.commit()
    login_user(user)
    next = request.values.get('next')
    if next != None and next.startsith('/'):
        return redirect(next)
    return redirect('/')

@home.route('/login/',methods=['GET','POST'])
def login():
    username=request.values.get('username').strip()
    password=request.values.get('password').strip()
    if username == '' or password =='':
        return redirect_wich_msg('/regloginpage/',u'用户名或者密码不能为空','reglogin')
    user=User.query.filter_by(username=username).first()
    if not user:
        return redirect_wich_msg('/regloginpage/',u'用户不存在','reglogin')
    m=hashlib.md5()
    m.update((password+user.salt).encode("utf8"))
    if(m.hexdigest() !=user.password):
        return redirect_wich_msg('/regloginpage/', u'密码错误', 'reglogin')
    login_user(user)
    next = request.values.get('next')
    if next != None  and next.startsith('/'):
        return redirect(next)
    return redirect('/')


@home.route('/logout/')
def logout():
    logout_user()
    return redirect('/')

@home.route('/upload',methods=['GET','POST'])
def upload():
    file=request.files['file']
    if file.filename.find('.')>0:
        file_ext=file.filename.rsplit('.',1)[1].strip().lower()
    if file_ext in instrgram.config['ALLOWED_EXT']:
        #保存文件
        filename= str()
    return redirect('/profile/%d'%current_user.id)